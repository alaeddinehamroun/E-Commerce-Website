...
export class CartService {
  ...
  constructor(private productService: ProductService,
    ...
    if (info !== null && info !== undefined && info.prodData[0].incart !== 0) {
      ...
      this.cartDataClient.prodData.forEach(p => {
        this.productService.getProduct(p.id).subscribe((actualProdInfo: ProductModelServer) => {
 ...
 CheckoutFromCart(userId: number) {
    ...
    this.httpClient.post<any>(`${this.ServerURL}/orders/payment`, null).subscribe({next: res  => {
      console.log(res.success)
      if (res.success) {
        this.resetServerData();
        this.httpClient.post<OrderConfirmationResponse>(`${this.ServerURL}/orders/new`, {
          userId: userId,
          products: this.cartDataClient.prodData
        }).subscribe({next: data => {

          this.orderService.getOrder(data.order_id).then(prods => {
            if (data.success) {
